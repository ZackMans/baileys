...
const makeMessagesSocket = (config) => {
    ...
    const alfarizi = new zckmns(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = alfarizi.detectType(content);
            ...
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await alfarizi.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await alfarizi.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await alfarizi.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        const albumContent = await alfarizi.handleAlbum(content, jid, quoted)
                        return albumContent;
                        
                    case 'EVENT':
                        return await alfarizi.handleEvent(content, jid, quoted)
                        
                    case 'POLL_RESULT':
                        return await alfarizi.handlePollResult(content, jid, quoted)
                }
            }
            ...
            if (options.msgId) fullMsg.key.id = options.msgId + '-' + require("crypto").randomBytes(6).toString("hex").toUpperCase();
